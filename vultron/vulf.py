...
for student in students:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            for f in records:
                ...
                if 'T' in d_raw and 'T' in t_raw:
                    f_id = f"{slug}_{d_raw}_{t_raw}"
                    cursor.execute("INSERT OR REPLACE INTO frequency VALUES (?,?,?,?,?)", (f_id, slug, d_raw.split('T')[0], t_raw.split('T')[1][:5], f.get('kategoriaFrekwencji')))
            conn.commit()
    except Exception as e: log(f"Błąd API frekwencji: {e}")
    ...
    try:
        ...
        if res_s.status_code == 200:
            ...
            for row in sj.get('statystyki', []):
                proc_s.append({"k": cat_m.get(row.get('kategoriaFrekwencji'), "Inna"), "m": {str(m.get('miesiac')): m.get('wartosc') for m in row.get('miesiace', [])}, "s1": row.get('okresy', [0,0])[0], "s2": row.get('okresy', [0,0])[1], "r": row.get('razem', 0)})
    ...
    requests.post(f"http://supervisor/core/api/states/sensor.vultron_freq_{slug}", headers={"Authorization": f"Bearer {HA_TOKEN}", "Content-Type": "application/json"}, json={"state": len(freq_data_ha), "attributes": {"wpisy": freq_data_ha, "friendly_name": f"Frekwencja: {display_name}", "last_update": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}}, timeout=10)
    if db_s:
        requests.post(f"http://supervisor/core/api/states/sensor.vultron_stats_{slug}", headers={"Authorization": f"Bearer {HA_TOKEN}", "Content-Type": "application/json"}, json={"state": db_s[0], "attributes": {"unit_of_measurement": "%", "rows": json.loads(db_s[1]), "friendly_name": f"Statystyki: {display_name}", "last_update": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}}, timeout=10)
