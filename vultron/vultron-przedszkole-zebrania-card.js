class VultronPrzedszkoleZebraniaCard extends HTMLElement {
  constructor() {
    super();
    this._uid = 'vpz-' + (
  ...
  renderHeader(state) {
    const name = state.attributes.friendly_name?.replace('Zebrania (przedszkole): ', '') || 'Dziecko';
  ...
  renderBody(state) {
    ...
    zebrania.forEach(z => {
      const shortTemat = (z.temat || "Brak tematu").length > 140
        ? z.temat.substring(0, 137) + '…'
        : (z.temat || "Brak tematu");
      ...
      html += `
        <div class="pzebrania-item ${isFuture ? 'pzebrania-future' : 'pzebrania-past'}" role="button" tabindex="0">
          <div style="flex:1;position:relative;padding-right:80px">
            ...
            <div style="font-size:0.95em;line-height:1.35;margin-bottom:8px;font-weight:600">
              ${this._esc(shortTemat)}
            </div>
  ...
  _showModal(z) {
    const titleEl = this.querySelector(`#${this._uid}-m-title`);
    titleEl.innerHTML = `<ha-icon icon="mdi:account-group"></ha-icon> ${this._esc(z.temat || 'Zebranie z rodzicami')}`;
    ...
    this.querySelector(`#${this._uid}-m-body`).textContent =
      z.agenda || "Brak szczegółowej agendy.";
...
customElements.define('vultron-przedszkole-zebrania-card', VultronPrzedszkoleZebraniaCard);
