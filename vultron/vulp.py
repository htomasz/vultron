...
for student in students:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            for lekcja in plan_raw:
                # 1. Pobranie statusu z pola adnotacja (Punkt 5 - .get())
                ...
                # Pobranie godziny z zabezpieczeniem (Punkt 5 - .get())
                godz_od = lekcja.get('godzinaOd', 'T ')[1][:5]
                godz_do = lekcja.get('godzinaDo', 'T ')[1][:5]
                godz_str = f"{godz_od}-{godz_do}"

                # Unikalny identyfikator dla bazy
                l_id = f"{slug}_{lekcja.get('data')}_{lekcja.get('godzinaOd')}"

                # ZAPIS DO BAZY
                cursor.execute("INSERT OR REPLACE INTO schedule VALUES (?,?,?,?,?,?,?,?)",
                    (l_id, slug, data_lekcji, godz_str, przedmiot,
    ...
    cursor.execute('''SELECT data, godzina, przedmiot, sala, prowadzacy, status
                      ...
                      ORDER BY data ASC, godzina ASC''', (slug, date_od_short, date_do_short))
